...
silver_dict = {
    "customers":"""
                   select 
                    ...
                    nascimento, 
                    profissao,
                    floor(datediff(now(), nascimento.member0)/365.25) as idade, 
                    case 
                        when floor(datediff(now(), nascimento.member0)/365.25) > 60 then 'idoso'
                        when floor(datediff(now(), nascimento.member0)/365.25) > 30 then 'adulto'
                        when floor(datediff(now(), nascimento.member0)/365.25) > 15 then 'jovem'
                        else 'crianca' 
...
gold_list = ["mysql/customers", "mysql/credit_score", "mysql/flight", "mysql/vehicle"]
